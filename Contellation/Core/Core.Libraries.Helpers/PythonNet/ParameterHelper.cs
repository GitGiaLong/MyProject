...
namespace Core.Libraries.Helpers.PythonNet
{
    ...
    class ParameterHelper : IEquatable<ParameterInfo>
    {
        ...
        public readonly Enums.PythonNet.ParameterModifier Modifier;
        public readonly ParameterHelper[]? GenericArguments;

        public ParameterHelper(ParameterInfo tp) : this(tp.ParameterType)
        {
            Modifier = Enums.PythonNet.ParameterModifier.None;

            if (tp.IsIn && tp.ParameterType.IsByRef)
            {
                Modifier = Enums.PythonNet.ParameterModifier.In;
            }
            else if (tp.IsOut && tp.ParameterType.IsByRef)
            {
                Modifier = Enums.PythonNet.ParameterModifier.Out;
            }
            else if (tp.ParameterType.IsByRef)
            {
                Modifier = Enums.PythonNet.ParameterModifier.Ref;
            }
        }
        ...
        public bool Equals(ParameterHelper other)
        {
            ...
            if (!(other.TypeName == TypeName && other.Modifier == Modifier)) { return false; }
