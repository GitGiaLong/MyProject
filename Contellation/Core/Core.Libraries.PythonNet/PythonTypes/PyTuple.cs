...
namespace Core.Libraries.PythonNet.PythonTypes
{
    ...
    public class PyTuple : PySequence
    {
        ...
        private static BorrowedReference FromObject(PyObject o)
        {
            ...
            if (!IsTupleType(o)) { throw new ArgumentException("object is not a tuple"); }
        ...
        private static StolenReference FromArray(PyObject[] items)
        {
            ...
            using var val = PyTuple_New(count);

            for (var i = 0; i < count; i++)
            {
                int res = PyTuple_SetItem(val.Borrow(), i, items[i]);
                if (res != 0)
                {
                    val.Dispose();
        ...
        public static bool IsTupleType(PyObject value)
        {
            ...
            return PyTuple_Check(value.obj);
        }
        ...
        public static PyTuple AsTuple(PyObject value)
        {
            ...
            NewReference op = PySequence_Tuple(value.Reference);
