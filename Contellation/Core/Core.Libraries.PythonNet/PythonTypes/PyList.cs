...
namespace Core.Libraries.PythonNet.PythonTypes
{
    ...
    public class PyList : PySequence
    {
        ...
        {
            if (o == null || !IsListType(o)) { throw new ArgumentException("object is not a list"); }
        ...
        /// </summary>
        public PyList() : base(PyList_New(0).StealOrThrow()) { }

        private static StolenReference FromArray(PyObject[] items)
        {
            if (items is null) { throw new ArgumentNullException(nameof(items)); }
            if (items.Any(item => item is null)) { throw new ArgumentException(message: Util.UseNone, paramName: nameof(items)); }

            int count = items.Length;
            using var val = PyList_New(count);

            for (var i = 0; i < count; i++)
            {
                int r = PyList_SetItem(val.Borrow(), i, new NewReference(items[i]).Steal());
                if (r < 0)
                {
                    val.Dispose();
        ...
        public static PyList AsList(PyObject value)
        {
            ...
            NewReference op = PySequence_List(value.Reference);
        ...
        public void Append(PyObject item)
        {
            ...
            int r = PyList_Append(this.Reference, item.Reference);
        ...
        public void Insert(int index, PyObject item)
        {
            ...
            int r = PyList_Insert(this, index, item);
            if (r < 0) { throw PythonException.ThrowLastAsClrException(); }
        }
        ...
        public void Reverse()
        {
            int r = PyList_Reverse(this.Reference);
            if (r < 0) { throw PythonException.ThrowLastAsClrException(); }
        }
        ...
        public void Sort()
        {
            int r = PyList_Sort(this.Reference);
            if (r < 0) { throw PythonException.ThrowLastAsClrException(); }
        }
