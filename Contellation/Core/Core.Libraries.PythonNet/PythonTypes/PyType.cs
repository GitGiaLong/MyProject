...
namespace Core.Libraries.PythonNet.PythonTypes
{
    ...
    {
        /// <summary>
        /// Creates heap type object from the <paramref name="spec"/>.
        /// </summary>
        ...
        internal PyType(BorrowedReference reference, bool prevalidated = false) : base(reference)
        {
            ...
            if (!PyType_Check(this)) { throw new ArgumentException("object is not a type"); }
        }
        ...
        /// <summary>
        /// Returns <c>true</c> when type is fully initialized
        /// </summary>
        public bool IsReady => Flags.HasFlag(TypeFlags.Ready);

        internal TypeFlags Flags
        {
            get { return (TypeFlags)Util.ReadCLong(this, TypeOffset.tp_flags); }
        ...
        {
            get { return GetBase(Reference); }
            set { ReplaceReference(this, TypeOffset.tp_base, new NewReference(value).Steal()); }
        }
        ...
        {
            IntPtr result = PyType_GetSlot(this.Reference, slot);
        ...
        private static StolenReference FromSpec(TypeSpec spec, PyTuple? bases = null)
        {
            ...
            var result = PyType_FromSpecWithBases(in nativeSpec, basesRef);
            // Runtime.PyErr_Print();
