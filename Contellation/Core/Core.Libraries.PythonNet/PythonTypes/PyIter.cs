...
namespace Core.Libraries.PythonNet.PythonTypes
{
    ...
    public class PyIter : PyObject, IEnumerator<PyObject>
    {
        ...
        public static PyIter GetIter(PyObject iterable)
        {
            ...
            var val = PyObject_GetIter(iterable.Reference);
        ...
        {
            NewReference next = PyIter_Next(Reference);
            if (next.IsNull())
            {
                if (Exceptions.ErrorOccurred()) { throw PythonException.ThrowLastAsClrException(); }
        ...
        protected PyIter(SerializationInfo info, StreamingContext context) : base(info, context)
        {
