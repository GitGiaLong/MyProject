...
namespace Core.Libraries.PythonNet.CollectionWrappers
{
    internal class SequenceWrapper<T> : IterableWrapper<T>, ICollection<T>
    {
        ...
        public int Count
        {
            get
            {
                var size = PySequence_Size(pyObject.Reference);
                if (size == -1) { CheckExceptionOccurred(); }
        ...
        public void Clear()
        {
            if (IsReadOnly) { throw new NotImplementedException(); }

            int result = PySequence_DelSlice(pyObject, 0, Count);

            if (result == -1) { CheckExceptionOccurred(); }
        }

        public bool Contains(T item)
        {
            //not sure if IEquatable is implemented and this will work!
            foreach (var element in this) { if (object.Equals(element, item)) { return true; } }
        ...
        protected bool removeAt(int index)
        {
            ...
            int result = PySequence_DelItem(pyObject, index);
