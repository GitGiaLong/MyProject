...
namespace Core.Libraries.PythonNet.Methods
{
    ...
    internal class MethodBinding : ExtensionType
    {
        ...
        public static NewReference mp_subscript(BorrowedReference tp, BorrowedReference idx)
        {
            ...
            MethodBase[] overloads = self.m.IsInstanceConstructor ? self.m.type.Value.GetConstructor(types) is { } ctor
                    ? new[] { ctor } : Array.Empty<MethodBase>() : MethodBinder.MatchParameters(self.m.info, types);
        ...
        PyObject Signature
        {
            get
            {
                ...
                Type type = infos.Select(i => i.DeclaringType).OrderByDescending(t => t, new TypeSpecificityComparer()).First();
                ...
                PyObject signatureClass = InspectModule.GetAttr("Signature");

                using var parameters = new PyList();
                using var parameterClass = primaryParameters.Length > 0 ? InspectModule.GetAttr("Parameter") : null;
                ...
                for (int i = 0; i < primaryParameters.Length; i++)
                {
                    ...
                    using var defaultValue = alternatives.Select(alternative => alternative!.DefaultValue != DBNull.Value ? alternative.DefaultValue.ToPython() : null)
                        .FirstOrDefault(v => v != null) ?? parameterClass?.GetAttr("empty");

                    if (alternatives.Any(alternative => alternative.Name != parameter.Name) || positionalOrKeyword is null)
                    {
                    ...
                    if (defaultValue is not null) { kw["default"] = defaultValue; }
        ...
        public static NewReference tp_getattro(BorrowedReference ob, BorrowedReference key)
        {
            ...
            switch (name)
            {
                ...
                case "Overloads":
                    ...
                    return om.Alloc();
                case "__signature__" when InspectModule is not null:
        ...
        public static NewReference tp_call(BorrowedReference ob, BorrowedReference args, BorrowedReference kw)
        {
            ...
            try
            {
                ...
                if (target is null && !self.m.IsStatic())
                {
                    ...
                    target = new PyObject(PyTuple_GetItem(args, 0));
                    disposeList.Add(target);

                    var unboundArgs = PyTuple_GetSlice(args, 1, len).MoveToPyObject();
                ...
                if (target is not null && PyObject_TYPE(target) != self.targetType!)
                {
                    ...
                    if (inst?.inst is IPythonDerivedType)
                    {
                        if (GetManagedObject(self.targetType!) is ClassBase baseType && baseType.type.Valid)
                        {
                            ...
                            using var baseMethod = PyObject_GetAttrString(target, baseMethodName);
            ...
            {
                foreach (var ptr in disposeList) { ptr.Dispose(); }
            }
        }
        ...
        public static NewReference tp_repr(BorrowedReference ob)
        {
            ...
            return PyString_FromString($"<{type} method '{name}'>");
        }
    }
}
