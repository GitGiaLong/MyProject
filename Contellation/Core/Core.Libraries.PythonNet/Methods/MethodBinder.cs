    ...
    internal class MethodBinder
    {
        ...
        internal static MethodInfo[] MatchParameters(MethodBase[] mi, Type[]? tp)
        {
            ...
            foreach (MethodInfo t in mi)
            {
                ...
                {
                    /* 
                     * MakeGenericMethod can throw ArgumentException 
                     * if the type parameters do not obey the constraints. 
                     */
        ...
        internal static MethodInfo? MatchSignatureAndParameters(MethodBase[] mi, Type[] genericTp, Type[] sigTp)
        {
            ...
            foreach (MethodInfo t in mi)
            {
                if (!t.IsGenericMethodDefinition) { continue; }

                Type[] genericArgs = t.GetGenericArguments();
                if (genericArgs.Length != genericCount) { continue; }

                ParameterInfo[] pi = t.GetParameters();
                if (pi.Length != signatureCount) { continue; }
        ...
        {
            if (mi == null) { return int.MaxValue; }
        ...
        internal Binding? Bind(BorrowedReference inst, BorrowedReference args, BorrowedReference kw, MethodBase? info, MethodBase[]? methodinfo)
        {
            ...
            if (kw != null)
            {
                nint pynkwargs = PyDict_Size(kw);
                using var keylist = PyDict_Keys(kw);
                using var valueList = PyDict_Values(kw);
                for (int i = 0; i < pynkwargs; ++i)
                {
                    var keyStr = GetManagedString(PyList_GetItem(keylist.Borrow(), i));
                    BorrowedReference value = PyList_GetItem(valueList.Borrow(), i);
        ...
        {
            var pynargs = (int)PyTuple_Size(args);
            var isGeneric = false;
            ...
            foreach (MethodBase mi in methods)
            {
                ...
                if (isReverse && OperatorMethod.IsComparisonOp((MethodInfo)mi)) { continue; } // Comparison operators in Python have no reverse mode.
                ...
                if (isOperator && !isReverse)
                {
                     ...
                     * Need to skip the first (left operand) CLR argument.
                     */
                ...
                else if (isOperator && isReverse)
                {
                     ...
                     * Need to take the first CLR argument.
                     */
                ...
                if (isOperator)
                {
                    if (inst != null)
                    {
                        if (ManagedType.GetManagedObject(inst) is CLRObject co)
                        {
                            ...
                            if (!isUnary) { margsTemp[passedOperandIndex] = margs[0]; }
                            margs = margsTemp;
                        }
                        else { continue; }
                    }
            ...
            if (argMatchedMethods.Count > 0)
            {
                ...
                for (int index = 0; index < argMatchedMethods.Count; index++)
                {
                    ...
                    {
                        bestCount++;
                        if (bestMatchIndex == -1) { bestMatchIndex = index; }
                    }
                }
            ...
            else if (matchGenerics && isGeneric)
            {
                 ...
                 */
                Type[]? types = PythonArgsToTypeArray(args, true);
        ...
        static BorrowedReference HandleParamsArray(BorrowedReference args, int arrayStart, int pyArgCount, out NewReference tempObject)
        {
            ...
            if ((pyArgCount - arrayStart) == 1)
            {
                 ...
                 */
                BorrowedReference item = PyTuple_GetItem(args, arrayStart);
                if (!PyString_Check(item) && PySequence_Check(item))
                {
                ...
                {
                    tempObject = PyTuple_GetSlice(args, arrayStart, pyArgCount);
        ...
        static object?[]? TryConvertArguments(ParameterInfo[] pi, bool paramsArray, BorrowedReference args, int pyArgCount,
            Dictionary<string, PyObject> kwargDict, ArrayList? defaultArgList, out int outs)
        {
            outs = 0;
            ...
            for (int paramIndex = 0; paramIndex < pi.Length; paramIndex++)
            {
                ...
                if (paramIndex >= pyArgCount && !(hasNamedParam || (paramsArray && paramIndex == arrayStart)))
                {
                    ...
                    if (parameter.ParameterType.IsByRef) { outs++; }
                ...
                else
                {
                    ...
                    {
                        op = PyTuple_GetItem(args, paramIndex);
                    }
                }
                                       ...
                                       out object? arg, out bool isOut)
        {
            ...
            if (!Converter.ToManaged(op, clrtype, out arg, true)) { return false; }
        ...
        static Type? TryComputeClrArgumentType(Type parameterType, BorrowedReference argument)
        {
            ...
            if (clrtype != null)
            {
                if ((parameterType != typeof(object)) && (parameterType != clrtype))
                {
                    ...
                    BorrowedReference pyoptype = PyObject_TYPE(argument);
                    var typematch = false;
                    if (pyoptype != null)
                    {
                        if (pytype != pyoptype)
        ...
        protected static void AppendArgumentTypes(StringBuilder to, BorrowedReference args)
        {
            ...
            {
                BorrowedReference arg = PyTuple_GetItem(args, argIndex);
                if (arg != null)
                {
                    BorrowedReference type = PyObject_TYPE(arg);
                    if (type != null)
                    {
                        using var description = PyObject_Str(type);
                        ...
                        {
                            to.Append(GetManagedString(description.Borrow()));
                        }
                    }
                }

                if (argIndex + 1 < argCount) { to.Append(", "); }
            }
            to.Append(')');
        }
        ...
        internal virtual NewReference Invoke(BorrowedReference inst, BorrowedReference args, BorrowedReference kw, MethodBase? info, MethodBase[]? methodinfo)
        {
            ...
            if (binding == null)
            {
                ...
                value.Append(": ");
                PyErr_Fetch(out var errType, out var errVal, out var errTrace);
                AppendArgumentTypes(to: value, args);
                PyErr_Restore(errType.StealNullable(), errVal.StealNullable(), errTrace.StealNullable());
                return Exceptions.RaiseTypeError(value.ToString());
            }

            if (allow_threads) { ts = PythonEngine.BeginAllowThreads(); }
            ...
            catch (Exception e)
            {
                ...
                if (allow_threads) { PythonEngine.EndAllowThreads(ts); }
             ...
             * return the out parameter as the result to Python 
             * (for code compatibility with ironpython). 
             */
            ...
            if (binding.outs > 0)
            {
                ...
                using var t = PyTuple_New(tupleSize);
                
                if (!isVoid)
                {
                    using var v = Converter.ToPython(result, returnType);
                    PyTuple_SetItem(t.Borrow(), n, v.Steal());
                    n++;
                }
                ...
                {
                    BorrowedReference item = PyTuple_GetItem(t.Borrow(), 0);
