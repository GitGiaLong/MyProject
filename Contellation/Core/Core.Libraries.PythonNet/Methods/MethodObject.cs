    ...
    internal class MethodObject : ExtensionType
    {
        ...
        public MethodObject(MaybeType type, string name, MethodBase[] info, bool allow_threads, bool argsReversed = false)
        {
            ...
            foreach (MethodBase item in info)
            {
                ...
                if (item.IsStatic) { this.is_static = true; }
            }
        ...
        public MethodObject WithOverloads(MethodBase[] overloads)  => new(type, name, overloads, allow_threads: binder.allow_threads);
        ...
        {
            if (doc is not null) { return new NewReference(doc); }
        ...
        internal NewReference GetName()
        {
            ...
            return PyString_FromString(names.First());
        }
        ...
        /// </remarks>
        internal bool IsStatic() { return is_static; }
        ...
        public static NewReference tp_getattro(BorrowedReference ob, BorrowedReference key)
        {
            ...
            if (PyUnicode_Compare(key, PyIdentifier.__doc__) == 0)
            {
        ...
        public static NewReference tp_descr_get(BorrowedReference ds, BorrowedReference ob, BorrowedReference tp)
        {
             ...
             */
            if (GetManagedObject(ob) is CLRObject obj && obj.inst.GetType() != self.type.Value
                && obj.inst is IPythonDerivedType && self.type.Value.IsInstanceOfType(obj.inst))
            {
        ...
        {
            var self = (MethodObject)GetManagedObject(ob)!;
            return PyString_FromString($"<method '{self.name}'>");
        }
        ...
        static bool AllowThreads(MethodBase[] methods)
        {
            ...
            foreach (var method in methods)
            {
                ...
                if (forbidsThreads) { hasForbidOverload = true; }
                else { hasAllowOverload = true; }
            }
