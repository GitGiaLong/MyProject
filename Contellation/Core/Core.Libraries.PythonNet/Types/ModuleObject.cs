...
#pragma warning disable CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
        /// <remarks>
        /// <seealso cref="dict"/> is initialized in <seealso cref="Create(string)"/>
        /// </remarks>
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
        ...
        public override NewReference Alloc()
        {
            ...
            if (dict is null)
            {
                ...
                {
                    if (!a.IsDynamic && a.Location != null) { filename = a.Location; }
                    docstring += "- " + a.FullName + "\n";
                }

                using var dictRef = PyObject_GenericGetDict(py.Borrow());
                dict = new PyDict(dictRef.StealOrThrow());
                using var pyname = PyString_FromString(moduleName);
                using var pyfilename = PyString_FromString(filename);
                using var pydocstring = PyString_FromString(docstring);
                BorrowedReference pycls = TypeManager.GetTypeReference(GetType());
                PyDict_SetItem(dict, PyIdentifier.__name__, pyname.Borrow());
                PyDict_SetItem(dict, PyIdentifier.__file__, pyfilename.Borrow());
                PyDict_SetItem(dict, PyIdentifier.__doc__, pydocstring.Borrow());
                PyDict_SetItem(dict, PyIdentifier.__class__, pycls);
            }
            else
            {
        ...
        public NewReference GetAttribute(string name, bool guess)
        {
            ...
            if (cached != null) { return new NewReference(cached); }

            Type type;

            //if (AssemblyManager.IsValidNamespace(name))
            //{
            //    IntPtr py_mod_name = PyString_FromString(name);
            //    IntPtr modules = PyImport_GetModuleDict();
            //    IntPtr module = PyDict_GetItem(modules, py_mod_name);
        ...
        {
            if (PyDict_SetItemString(dict, name, ob) != 0)
            {
        ...
        public void LoadNames()
        {
            foreach (string name in AssemblyManager.GetNames(_namespace))
            {
                ...
                if (m != null) { continue; }
                BorrowedReference attr = PyDict_GetItemString(dict, name);
                ...
                if (!attrVal.IsNull())
                {
                    ...
                    using var pyname = PyString_FromString(name);
                    if (PyList_Append(__all__, pyname.Borrow()) != 0)
                    {
                        throw PythonException.ThrowLastAsClrException();
                    }
                }
        ...
        internal void ResetModuleMembers()
        {
            ...
            var methods = type.GetMethods(ModuleMethodFlags)
                .Where(m => m.GetCustomAttribute<ModuleFunctionAttribute>() is not null).OfType<MemberInfo>();
            ...
            {
                if (PyDict_DelItemString(dict, memberName) != 0)
                {
        ...
        public static NewReference tp_getattro(BorrowedReference ob, BorrowedReference key)
        {
            ...
            BorrowedReference op = PyDict_GetItem(self.dict, key);
            if (op != null) { return new NewReference(op); }

            string? name = InternString.GetManagedString(key);
            if (name == "__dict__") { return new NewReference(self.dict); }
        ...
        {
            var self = (ModuleObject)GetManagedObject(ob)!;
            return PyString_FromString($"<module '{self.moduleName}'>");
        }
        ...
        {
            var managedKey = GetManagedString(key);
            if ((settableAttributes.Contains(managedKey)) || (ManagedType.GetManagedObject(val) is ModuleObject))
            {
                var self = (ModuleObject)ManagedType.GetManagedObject(ob)!;
                return PyDict_SetItem(self.dict, key, val);
            }
        ...
        protected override Dictionary<string, object?>? OnSave(BorrowedReference ob)
        {
            ...
            {
                if ((PyDict_DelItemString(dict, pair.Key) == -1) &&
