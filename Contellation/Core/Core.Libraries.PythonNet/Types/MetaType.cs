...
using Core.Libraries.PythonNet.Native;
...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
        ...
        {
            if (HostedInPython) { _metaSlotsHodler.ResetSlots(); }
        ...
        {
            var len = PyTuple_Size(args);
            if (len < 3) { return Exceptions.RaiseTypeError("invalid argument list"); }

            BorrowedReference name = PyTuple_GetItem(args, 0);
            BorrowedReference bases = PyTuple_GetItem(args, 1);
            BorrowedReference dict = PyTuple_GetItem(args, 2);
            ...
            var baseClassCount = PyTuple_Size(bases);
            if (baseClassCount == 0) { return Exceptions.RaiseTypeError("zero base classes "); }

            for (nint i = 0; i < baseClassCount; i++)
            {
                var baseTypeIt = PyTuple_GetItem(bases, (int)i);
                ...
                if (GetManagedObject(baseTypeIt) is ClassBase classBaseIt)
                {
                    ...
                    if (classBaseIt.type.Value.IsInterface) { interfaces.Add(classBaseIt.type.Value); }
            ...
            if (baseTypes.Count == 0) { baseTypes.Add(new ClassBase(typeof(object))); }
            ...
            if (interfaces.Distinct().Count() != interfaces.Count)
            {
                ...
                var duplicateTypes = interfaces.GroupBy(type => type).Where(typeGroup => typeGroup.Count() > 1)
            ...
            BorrowedReference slots = PyDict_GetItem(dict, PyIdentifier.__slots__);
            if (slots != null)
            {
            ...
            /* 
             * If __assembly__ or __namespace__ are in the class dictionary then create a managed sub type. 
             * This creates a new managed type that can be used from .net to call back into python. 
             */
            if (null != dict)
            {
            ...
            var base_type = PyTuple_GetItem(bases, 0);

            // otherwise just create a basic type without reflecting back into the managed side.
            IntPtr func = Util.ReadIntPtr(PyTypeType, TypeOffset.tp_new);
            NewReference type = NativeCall.Call_3(func, tp, args, kw);
            if (type.IsNull()) { return default; }

            var flags = PyType.GetFlags(type.Borrow());
            if (!flags.HasFlag(TypeFlags.Ready)) { throw new NotSupportedException("PyType.tp_new returned an incomplete type"); }
            ...
            Debug.Assert(clrInstOffset > 0 && clrInstOffset < Util.ReadInt32(type.Borrow(), TypeOffset.tp_basicsize));
            ...
            PyType_Modified(type.Borrow());

            return type;
        }

        public static NewReference tp_alloc(BorrowedReference mt, nint n) => PyType_GenericAlloc(mt, n);


        public static void tp_free(NewReference tp) { PyObject_GC_Del(tp.Steal()); }
        ...
        public static NewReference tp_call(BorrowedReference tp, BorrowedReference args, BorrowedReference kw)
        {
            ...
            if (tp_new == IntPtr.Zero) { return Exceptions.RaiseTypeError("invalid object"); }
            ...
            return type.Init(obj.Borrow(), args, kw) ? obj.Move() : default;
        }
        ...
        {
            BorrowedReference descr = _PyType_Lookup(tp, name);

            if (descr != null)
            {
                BorrowedReference dt = PyObject_TYPE(descr);

                if (dt == PyWrapperDescriptorType || dt == PyMethodType || typeof(ExtensionType).IsInstanceOfType(GetManagedObject(descr)))
                {
                    IntPtr fp = Util.ReadIntPtr(dt, TypeOffset.tp_descr_set);
                    if (fp != IntPtr.Zero) { return NativeCall.Int_Call_3(fp, descr, name, value); }
                    Exceptions.SetError(Exceptions.AttributeError, "attribute is read-only");
                    return -1;
                }
            }

            int res = PyObject_GenericSetAttr(tp, name, value);
            PyType_Modified(tp);
        ...
        {
            if (GetManagedObject(tp) is ClassBase cb) { return cb.type_subscript(idx); }
            return Exceptions.RaiseTypeError("unsubscriptable object");
        }

        /// <summary>
        /// Dealloc implementation. This is called when a Python type generated
        /// by this metatype is no longer referenced from the Python 
        /// </summary>
        public static void tp_dealloc(NewReference lastRef)
        {
            var weakrefs = PyObject_GetWeakRefList(lastRef.Borrow());
            if (weakrefs != null) { PyObject_ClearWeakRefs(lastRef.Borrow()); }
            ...
            var op = PyObject_TYPE(lastRef.Borrow());
            Debug.Assert(PyCLRMetaType is null || PyCLRMetaType == op);
            var builtinType = PyObject_TYPE(PyObject_TYPE(op));
            ...
            /*
             * must decref type. 
             * type_dealloc from PyType will use it to get tp_free so must keep the value 
             */
        ...
        {
            if (GetManagedObject(tp) is not ClassBase cb || !cb.type.Valid) { return new NewReference(PyFalse); }

            using var argsObj = new PyList(args);
            if (argsObj.Length() != 1) { return Exceptions.RaiseTypeError("Invalid parameter count"); }

            PyObject arg = argsObj[0];
            var otherType = checkType ? arg : arg.GetPythonType();

            if (PyObject_TYPE(otherType) != PyCLRMetaType) { return new NewReference(PyFalse); }
