...
namespace Core.Libraries.PythonNet.Types
{
    ...
    internal class ClassDerivedObject : ClassObject
    {
        ...
        internal static NewReference ToPython(IPythonDerivedType obj)
        {
            ...
            try
            {
                self = PythonDerivedType.GetPyObj(obj).CheckRun();
            }
            ...
            string moduleName = "Python.Runtime.Dynamic.dll")
        {
            ...
            // FIXME: fb not used
            FieldBuilder fb = typeBuilder.DefineField(PythonDerivedType.PyObjName,
...
#pragma warning disable CS0618 // PythonDerivedType is for internal use only
            il.Emit(OpCodes.Call, typeof(PythonDerivedType).GetMethod(nameof(PythonDerivedType.PyFinalize)));
#pragma warning restore CS0618 // PythonDerivedType is for internal use only
        ...
        private static void AddConstructor(ConstructorInfo ctor, Type baseType, TypeBuilder typeBuilder)
        {
            ...
            il.Emit(OpCodes.Ldloc_0);
#pragma warning disable CS0618 // PythonDerivedType is for internal use only
            il.Emit(OpCodes.Call, typeof(PythonDerivedType).GetMethod(nameof(PythonDerivedType.InvokeCtor)));
#pragma warning restore CS0618 // PythonDerivedType is for internal use only
            il.Emit(OpCodes.Ret);
        }
...
#pragma warning disable CS0618 // PythonDerivedType is for internal use only
            ...
            {
                il.Emit(OpCodes.Call, typeof(PythonDerivedType).GetMethod(nameof(PythonDerivedType.InvokeMethodVoid)));
            }
            else
            {
                il.Emit(OpCodes.Call,
                    typeof(PythonDerivedType).GetMethod(nameof(PythonDerivedType.InvokeMethod)).MakeGenericMethod(method.ReturnType));
            }
#pragma warning restore CS0618 // PythonDerivedType is for internal use only
