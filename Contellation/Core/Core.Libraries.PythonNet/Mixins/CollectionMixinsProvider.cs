...
namespace Core.Libraries.PythonNet.Mixins
{
    class CollectionMixinsProvider : IPythonBaseTypeProvider, IDisposable
    {
        ...
        public IEnumerable<PyType> GetBaseTypes(Type type, IList<PyType> existingBases)
        {
            ...
            // item collections
            if (interfaces.Contains(typeof(IList<>)) || interfaces.Contains(typeof(System.Collections.IList)))
            {
        ...
        static Type[] NewInterfaces(Type type)
        {
            ...
            return type.BaseType != null ? result.Except(type.BaseType.GetInterfaces()).ToArray() : result;
        }

        static Type GetDefinition(Type type) => type.IsGenericType ? type.GetGenericTypeDefinition() : type;

        public void Dispose()
        {
            if (this.mixinsModule.IsValueCreated) { this.mixinsModule.Value.Dispose(); }
        }
    }
}
