...
namespace Core.Libraries.PythonNet.Python
{
    ...
    {
        public PythonException(PyType type, PyObject? value, PyObject? traceback, string message, Exception? innerException)
        ...
        public PythonException(PyType type, PyObject? value, PyObject? traceback, Exception? innerException)
        ...
        internal static PythonException? FetchCurrentOrNullRaw()
        {
            ...
            PyErr_Fetch(type: out var type, val: out var value, tb: out var traceback);
        ...
        internal static PythonException FetchCurrentRaw() => FetchCurrentOrNullRaw()
               ?? throw new InvalidOperationException("No exception is set");
        ...
        internal static Exception? PeekCurrentOrNull(out ExceptionDispatchInfo? dispatchInfo)
        {
            ...
            PyErr_Fetch(out var type, out var value, out var traceback);
            PyErr_Restore(new NewReference(type, canBeNull: true).StealNullable(),
            ...
            PyErr_Restore(type.StealNullable(), value.StealNullable(), traceback.StealNullable());
        ...
        internal static Exception? FetchCurrentOrNull(out ExceptionDispatchInfo? dispatchInfo)
        {
            ...
            PyErr_NormalizeException(type: ref type, val: ref value, tb: ref traceback);
        ...
        private static ExceptionDispatchInfo? TryGetDispatchInfo(BorrowedReference exception)
        {
            ...
            using var pyInfo = PyObject_GetAttrString(exception, Exceptions.DispatchInfoAttribute);
            if (pyInfo.IsNull())
            {
                                           ...
                                           out ExceptionDispatchInfo? exceptionDispatchInfo)
        {
            ...
            using var cause = PyException_GetCause(valRef);
        ...
        {
            using var pyErrType = InteropModule.GetAttr("PyErr");
        ...
        private static Exception? FromCause(BorrowedReference cause)
        {
            ...
            Debug.Assert(PyObject_TypeCheck(cause, Exceptions.BaseException));

            using var innerTraceback = PyException_GetTraceback(cause);
            return FromPyErr(typeRef: PyObject_TYPE(cause), valRef: cause,
                tbRef: innerTraceback.BorrowNullable(), out _);
        ...
        internal void Restore()
        {
            ...
            PyErr_Restore(type: type.Steal(), val: value.StealNullable(), tb: traceback.StealNullable());
        }
        ...
        public bool IsNormalized
        {
            get
            {
                ...
                using var _ = new Py.GILState();
                return PyObject_TypeCheck(Value.Reference, Type.Reference);
            }
        }
        ...
        public void Normalize()
        {
            ...
            try
            {
                ...
                /* 
                 * If an error is set and this PythonException is unnormalized, 
                 * the error will be cleared and the PythonException 
                 * will be replaced by a different error. 
                 */
                NewReference value = Value.NewReferenceOrNull();
                NewReference type = Type.NewReferenceOrNull();
                NewReference tb = Traceback.NewReferenceOrNull();

                PyErr_NormalizeException(type: ref type, val: ref value, tb: ref tb);
                ...
                try
                {
                    ...
                    if (!tb.IsNull())
                    {
                        ...
                        int r = PyException_SetTraceback(Value.Reference, tb.Borrow());
                        ThrowIfIsNotZero(r);
                    }
                }
                finally
        ...
        public string Format()
        {
            ...
            if (copy.Traceback is null || copy.Value is null) { return StackTrace; }
        ...
        {
            return PyErr_GivenExceptionMatches(given: (Value ?? Type).Reference, typeOrTypes: type) != 0;
        }
