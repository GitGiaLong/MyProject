...
namespace Libraries.Custom.Helpers
{
    internal static class DateTimeHelper
    {
        ...
        {
            try { return Cal.AddDays(time, days); }
            catch (System.ArgumentException) { return null; }
        }

        public static DateTime? AddMonths(DateTime time, int months)
        {
            try { return Cal.AddMonths(time, months); }
            catch (System.ArgumentException) { return null; }
        }

        public static DateTime? AddYears(DateTime time, int years)
        {
            try { return Cal.AddYears(time, years); }
            catch (System.ArgumentException) { return null; }
        }

        public static DateTime? SetYear(DateTime date, int year) { return DateTimeHelper.AddYears(date, year - date.Year); }

        public static DateTime? SetYearMonth(DateTime date, DateTime yearMonth)
        {
            DateTime? target = SetYear(date, yearMonth.Year);
            if (target.HasValue) { target = DateTimeHelper.AddMonths(target.Value, yearMonth.Month - date.Month); }

            return target;
        }

        public static int CompareDays(DateTime dt1, DateTime dt2) { return DateTime.Compare(DiscardTime(dt1), DiscardTime(dt2)); }

        public static int CompareYearMonth(DateTime dt1, DateTime dt2) { return ((dt1.Year - dt2.Year) * 12) + (dt1.Month - dt2.Month); }

        public static int DecadeOfDate(DateTime date) { return date.Year - (date.Year % 10); }

        public static DateTime DiscardDayTime(DateTime d) { return new DateTime(d.Year, d.Month, 1, 0, 0, 0); }

        public static DateTime DiscardTime(DateTime d) { return d.Date; }

        public static int EndOfDecade(DateTime date) { return DecadeOfDate(date) + 9; }

        public static DateTimeFormatInfo GetCurrentDateFormat() { return GetDateFormat(CultureInfo.CurrentCulture); }

        internal static DateTimeFormatInfo GetDateFormat(CultureInfo culture)
        {
            if (culture.Calendar is GregorianCalendar) { return culture.DateTimeFormat; }
        ...
        /// <param name="range"><see cref="CalendarDateRange"/></param>
        /// <returns></returns>
        public static bool InRange(DateTime date, CalendarDateRange range) { return InRange(date, range.Start, range.End); }

        /// <summary>
        /// returns true if the date is included in the range
        /// </summary>
        /// <param name="date"><see cref="DateTime"/></param>
        /// <param name="start"><see cref="DateTime"/></param>
        /// <param name="end"><see cref="DateTime"/></param>
        /// <returns></returns>
        public static bool InRange(DateTime date, DateTime start, DateTime end)
        {
            ...
            if (CompareDays(date, start) > -1 && CompareDays(date, end) < 1) { return true; }
        ...
        public static string ToDayString(DateTime? date, CultureInfo culture)
        {
            ...
            if (date.HasValue && format is not null) { result = date.Value.Day.ToString(format); }
        ...
        public static string ToYearMonthPatternString(DateTime? date, CultureInfo culture)
        {
            ...
            if (date.HasValue && format != null) { result = date.Value.ToString(format.YearMonthPattern, format); }
        ...
        public static string ToYearString(DateTime? date, CultureInfo culture)
        {
            ...
            if (date.HasValue && format != null) { result = date.Value.Year.ToString(format); }
        ...
        public static string ToAbbreviatedMonthString(DateTime? date, CultureInfo culture)
        {
            ...
            if (date.HasValue && format is not null)
            {
                ...
                if (monthNames is not null && monthNames.Length > 0) { result = monthNames[(date.Value.Month - 1) % monthNames.Length]; }
            }
        ...
        public static string ToLongDateString(DateTime? date, CultureInfo culture)
        {
            ...
            if (date.HasValue && format is not null) { result = date.Value.Date.ToString(format.LongDatePattern, format); }
