using Libraries.Custom.Enums.Control.Virtualizings;
using Libraries.Custom.Structs.Virtualizings;
...
namespace Libraries.Custom.Controls
{
    ...
    public class VirtualizingWrapPanel : VirtualizingPanelBase
    {
        ...
        public static readonly DependencyProperty SpacingModeProperty = DependencyProperty.Register(nameof(SpacingMode),
            typeof(SpacingMode), typeof(VirtualizingWrapPanel), new FrameworkPropertyMetadata(SpacingMode.Uniform, FrameworkPropertyMetadataOptions.AffectsMeasure));
        ...
        public static readonly DependencyProperty OrientationProperty = DependencyProperty.Register(nameof(Orientation),
            typeof(Orientation), typeof(VirtualizingWrapPanel), new FrameworkPropertyMetadata(Orientation.Vertical, FrameworkPropertyMetadataOptions.AffectsMeasure, OnOrientationChanged));
        ...
        public static readonly DependencyProperty ItemSizeProperty = DependencyProperty.Register(nameof(ItemSize),
            typeof(Size), typeof(VirtualizingWrapPanel), new FrameworkPropertyMetadata(Size.Empty, FrameworkPropertyMetadataOptions.AffectsMeasure));
        ...
        {
            get { return (bool)GetValue(StretchItemsProperty); }
        ...
        public static readonly DependencyProperty StretchItemsProperty = DependencyProperty.Register(nameof(StretchItems),
            typeof(bool), typeof(VirtualizingWrapPanel), new FrameworkPropertyMetadata(false, FrameworkPropertyMetadataOptions.AffectsArrange));
        ...
        {
            if (ItemsOwner is IHierarchicalVirtualizationAndScrollInfo groupItem && VirtualizingPanel.GetIsVirtualizingWhenGrouping(ItemsControl))
            {
            ...
            if (ItemSize != Size.Empty) { ChildSize = ItemSize; }
            else if (InternalChildren.Count != 0) { ChildSize = InternalChildren[0].DesiredSize; }
            else { ChildSize = CalculateChildSize(availableSize); }

            ItemsPerRowCount = double.IsInfinity(GetWidth(availableSize)) ? Items.Count
        ...
        protected override Size CalculateExtent(Size availableSize)
        {
            var extentWidth = SpacingMode != SpacingMode.None && !double.IsInfinity(GetWidth(availableSize))
                ? GetWidth(availableSize) : GetWidth(ChildSize) * ItemsPerRowCount;

            if (ItemsOwner is IHierarchicalVirtualizationAndScrollInfo)
            {
                extentWidth = Orientation == Orientation.Vertical ? Math.Max(extentWidth - (Margin.Left + Margin.Right), 0)
        ...
        protected override Size ArrangeOverride(Size finalSize)
        {
            ...
            if (ItemsOwner is IHierarchicalVirtualizationAndScrollInfo) { offsetY = 0; }
        ...
        {
            if (!StretchItems) { return ChildSize; }
        ...
        {
            var value = ItemsControl.ItemContainerStyle?.Setters.OfType<Setter>().FirstOrDefault(setter => setter.Property == property) ?.Value;
        ...
        {
            if (!IsVirtualizing) { return new ItemRange(0, Items.Count - 1); }
            ...
            if (ItemsOwner is IHierarchicalVirtualizationAndScrollInfo groupItem)
            {
                ...
                if (CacheLengthUnit == VirtualizationCacheLengthUnit.Pixel)
                {
                    ...
                    var cacheAfterInPixel = Math.Min(CacheLength.CacheAfterViewport,
        ...
        protected override void BringIndexIntoView(int index)
        {
            ...
            if (ItemsPerRowCount == 0) { throw new InvalidOperationException(); }

            var offset = (index / ItemsPerRowCount) * GetHeight(ChildSize);

            if (Orientation == Orientation.Horizontal) { SetHorizontalOffset(offset); }
            else { SetVerticalOffset(offset); }
        }
