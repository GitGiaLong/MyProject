...
namespace Libraries.Custom.Controls.Gravatars
{
    public class GravatarGenerator : IGravatar
    {
        ...
        private string GetHashCode(string id)
        {
            ...
            foreach (var item in hash) { stringBuilder.Append(item.ToString("X2")); }
        ...
        private Color Hsl2Rgb(double h, double s = 0.7, double b = 0.5)
        {
            ...
            return Color.FromRgb((byte)(arr[hValue % 6] * 255), (byte)(arr[(hValue | 16) % 6] * 255), (byte)(arr[(hValue | 8) % 6] * 255));
        }
    }
}
