...
namespace Core.Libraries.WPF.Controls
{
    ...
    {
        private readonly EventIdentifier _verticalIdentifier = new EventIdentifier();

        private readonly EventIdentifier _horizontalIdentifier = new EventIdentifier();
        ...
        {
            get { return (bool)GetValue(IsScrollingVerticallyProperty); }
        ...
        {
            get { return (bool)GetValue(IsScrollingHorizontallyProperty); }
        ...
        {
            get { return (double)GetValue(MinimalChangeProperty); }
        ...
        public static readonly DependencyProperty TimeoutProperty = DependencyProperty.Register(nameof(Timeout), typeof(int),
            typeof(DynamicScrollViewer), new PropertyMetadata(1200, OnTimeoutChanged));
        ...
        private async void UpdateVerticalScrollingState()
        {
            ...
            if (_timeout > -1) { await Task.Delay(_timeout < 10000 ? _timeout : 1000); }

            if (_verticalIdentifier.IsEqual(currentEvent) && _scrollingVertically) { SetCurrentValue(IsScrollingVerticallyProperty, false); }
        }
        ...
        private async void UpdateHorizontalScrollingState()
        {
            ...
            if (!_scrollingHorizontally) { SetCurrentValue(IsScrollingHorizontallyProperty, true); }

            await Task.Delay(Timeout < 10000 ? Timeout : 1000);

            if (_horizontalIdentifier.IsEqual(currentEvent) && _scrollingHorizontally) { SetCurrentValue(IsScrollingHorizontallyProperty, false); }
        }

        private static void OnIsScrollingVerticallyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            if (d is not DynamicScrollViewer scroll) { return; }
