...
using Core.Libraries.WPF.Interactivities.Layouts;
...
namespace Core.Libraries.WPF.Controls.Datetimes.TimeBar
{
    internal class MouseDragElementBehaviorEx : Behavior<FrameworkElement>
    {
        public double X
        {
            get { return (double)GetValue(MouseDragElementBehavior.XProperty); }
        ...
        public double Y
        {
            get { return (double)GetValue(MouseDragElementBehavior.YProperty); }
        ...
        {
            get { return (bool)GetValue(MouseDragElementBehavior.ConstrainToParentBoundsProperty); }
            set { SetValue(MouseDragElementBehavior.ConstrainToParentBoundsProperty, value); }
        }
        public static readonly DependencyProperty ConstrainToParentBoundsProperty = DependencyProperty.Register(nameof(ConstrainToParentBounds), typeof(bool),
        ...
        private UIElement RootElement
        {
            get
            {
                ...
                for (var dependencyObject = reference; dependencyObject != null; dependencyObject = VisualTreeHelper.GetParent(reference)) reference = dependencyObject;
        ...
        private Transform RenderTransform
        {
            ...
            set
            {
                if (Equals(_cachedRenderTransform, value)) { return; }
        ...
        {
            if (_settingPosition || AssociatedObject == null) { return; }
        ...
        internal void ApplyTranslationTransform(double x, double y)
        {
            ...
            if (translateTransform == null)
            {
                ...
                {
                    if (transformGroup1.Children.Count > 0) { translateTransform = transformGroup1.Children[transformGroup1.Children.Count - 1] as TranslateTransform; }
                    if (translateTransform == null)
                    {
                ...
                else
                {
                    if (matrixTransform != null)
                    {
                        ...
                        /// Microsoft's class has been modified there
                        if (!LockX) { matrix.OffsetX += x; }
                        if (!LockY) { matrix.OffsetY += y; }
                        /// End of modification
                        RenderTransform = new MatrixTransform { Matrix = matrix };
                        return;
                    }
                    var transformGroup2 = new TransformGroup();
                    translateTransform = new TranslateTransform();
                    
                    if (renderTransform != null) { transformGroup2.Children.Add(renderTransform); }
            ...
            /// Microsoft's class has been modified there
            if (!LockX) { translateTransform.X += x; }
            if (!LockY) { translateTransform.Y += y; }
            /// End of modification
        }
        ...
        internal static Transform CloneTransform(Transform transform)
        {
            ...
            if (transform is MatrixTransform matrixTransform) { return new MatrixTransform { Matrix = matrixTransform.Matrix }; }
            if (!(transform is TransformGroup transformGroup1)) { return null; }
            
            var transformGroup2 = new TransformGroup();
            
            foreach (var child in transformGroup1.Children) { transformGroup2.Children.Add(CloneTransform(child)); }
        ...
        {
            var point = TransformAsVector(AssociatedObject.TransformToVisual(RootElement), newPositionInElementCoordinates.X - _relativePosition.X,
        ...
        {
            if (rect1.IsEmpty || rect2.IsEmpty || rect1.X > rect2.X || rect1.Y > rect2.Y || rect1.X + rect1.Width < rect2.X + rect2.Width) { return false; }
