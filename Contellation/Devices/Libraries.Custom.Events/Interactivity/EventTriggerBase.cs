...
namespace Libraries.Custom.Events
{
    public abstract class EventTriggerBase : Interactivities.TriggerBase
    {
        ...
        public static readonly DependencyProperty SourceObjectProperty = DependencyProperty.Register(nameof(SourceObject), typeof(object),
            typeof(EventTriggerBase), new PropertyMetadata(new PropertyChangedCallback(OnSourceObjectChanged)));
        ...
        public static readonly DependencyProperty SourceNameProperty = DependencyProperty.Register(nameof(SourceName), typeof(string),
            typeof(EventTriggerBase), new PropertyMetadata(new PropertyChangedCallback(OnSourceNameChanged)));
                ...
                else if (IsSourceNameSet)
                {
                    ...
                    if (source != null && !SourceTypeConstraint.IsAssignableFrom(source.GetType()))
                    {
                        throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture,
                            ...
                            source.GetType(), SourceTypeConstraint, "Source"));
                    }
                }
                return source;
            }
        ...
        private void RegisterEvent(object obj, string eventName)
        {
            ...
            if (eventInfo == null)
            {
                if (SourceObject != null)
                {
                    throw new ArgumentException(string.Format(CultureInfo.CurrentCulture,
                        ExceptionStringTable.EventTriggerCannotFindEventNameExceptionMessage, eventName, obj.GetType().Name));
                }
                else
                {
                    return;
            ...
            if (!IsValidEvent(eventInfo))
            {
                ...
                {
                    throw new ArgumentException(string.Format(CultureInfo.CurrentCulture,
                        ExceptionStringTable.EventTriggerBaseInvalidEventExceptionMessage, eventName, obj.GetType().Name));
                }
                else
                {
                    return;
