...
namespace Core.WPF.Appearance
{
    ...
    internal class ResourceDictionaryManager
    {
        ...
        public ResourceDictionary? GetDictionary(string resourceLookup)
        {
            ...
            if (applicationDictionaries.Count == 0) { return null; }
            ...
            foreach (ResourceDictionary t in applicationDictionaries)
            {
                ...
                if (t?.Source != null)
                {
                    ...
                    if (resourceDictionaryUri.Contains(SearchNamespace) && resourceDictionaryUri.Contains(resourceLookup)) { return t; }
                }

                foreach (ResourceDictionary? t1 in t!.MergedDictionaries)
                {
                    if (t1?.Source == null) { continue; }

                    resourceDictionaryUri = t1.Source.ToString().ToLower().Trim();

                    if (!resourceDictionaryUri.Contains(SearchNamespace) || !resourceDictionaryUri.Contains(resourceLookup)) { continue; }
        ...
        {
            Collection<ResourceDictionary> applicationDictionaries = UiApplication.Current.Resources.MergedDictionaries;

            if (applicationDictionaries.Count == 0 || newResourceUri is null) { return false; }
            ...
            for (var i = 0; i < applicationDictionaries.Count; i++)
            {
                ...
                {
                    if (applicationDictionaries[i].MergedDictionaries[j]?.Source == null) { continue; }

                    sourceUri = applicationDictionaries[i].MergedDictionaries[j].Source.ToString().ToLower().Trim();

                    if (!sourceUri.Contains(SearchNamespace) || !sourceUri.Contains(resourceLookup)) { continue; }
